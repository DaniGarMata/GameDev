...
bool Map::Load()
{
    ...
    mapLayerItem = mapData.maplayers.start;
    while (mapLayerItem != NULL) {
        if (mapLayerItem->data->properties.GetProperty("Collide") != NULL && mapLayerItem->data->properties.GetProperty("Collide")->value) {
            ...
            for (int x = 0; x < mapLayerItem->data->width; x++)
            {
                for (int y = 0; y < mapLayerItem->data->height; y++)
                {
                    ...
                    int gid = mapLayerItem->data->Get(x, y);
                    if (gid != 0) {
                        app->physics->CreateRectangle(x * 32 + 16, y * 32 + 16, 32, 32, STATIC);
                    }
                }
            }
        }
