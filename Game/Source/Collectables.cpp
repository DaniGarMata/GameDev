...
bool Collectables::Start()
{
	...
	SString tmp("%s%s", sfx.GetString(), "coin.wav");
	SString tmp2("%s%s", sfx.GetString(), "mushroom.wav");
	coinSFX = app->audio->LoadFx(tmp.GetString());
	mushroomSFX = app->audio->LoadFx(tmp2.GetString());
	return true;
}
